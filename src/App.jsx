...
const getNestedVal = (obj, path) => {
  ...
  return path.split(".").reduce((o, k) => o?.[k] ?? 0, obj);
};

const calcGoalProgress = (goal, history, patrimoinePocheTotal) => {
  ...
  if (src.id === "patrimoine") {
    const val = patrimoinePocheTotal;
    if (goal.reverse) return Math.min(100, Math.max(0, Math.round(((val - (goal.startValue || 0)) / (target - (goal.startValue || 0))) * 100)));
    return Math.min(100, Math.max(0, Math.round((val / target) * 100)));
  }

  const relevantDays = history.filter(d => {
  ...
  if (src.isDaily) {
    ...
    relevantDays.forEach(d => {
      ...
      if (goal.reverse) { if (val > 0 && val <= target) successDays++; }
      else { if (val >= target) successDays++; }
    });
    return Math.min(100, Math.round((successDays / relevantDays.length) * 100));
  } else {
    const lastEntry = [...history].reverse().find(d => getNestedVal(d, src.path) > 0);
    const val = lastEntry ? getNestedVal(lastEntry, src.path) : 0;
    if (goal.reverse) {
      const startVal = goal.startValue || val;
      return Math.min(100, Math.max(0, Math.round(((startVal - val) / (startVal - target)) * 100)));
    }
    return Math.min(100, Math.max(0, Math.round((val / target) * 100)));
  }
};
...
function getIntelligence(history) {
  ...
  if (!todayRec && lastSportType) {
    ...
    if (lastIdx >= 0) { todayRec = `PPL ${ppl[(lastIdx + 1) % 3]}`; advice.push(`💪 Dernière séance : ${lastSportType} → Recommandation : ${todayRec}`); }
  }
...
// ── ICONS SVG ──────────────────────────────────────────────────────────────
const Icons = {
  home: (color) => <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>,
  track: (color) => <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>,
  money: (color) => <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>,
  goals: (color) => <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>,
  stats: (color) => <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>,
  profile: (color) => <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>,
};
...
// ── COMPOSANTS ─────────────────────────────────────────────────────────────
const inp = { background: C.surfaceAlt, border: `1px solid ${C.border}`, borderRadius: 10, padding: "11px 14px", color: C.text, fontSize: 14, outline: "none", width: "100%", boxSizing: "border-box", fontFamily: "Inter, sans-serif" };
  ...
  return (
    <div style={{ background: C.surface, border: `2px solid ${color}33`, borderRadius: 16, padding: 16, marginBottom: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <p style={{ fontSize: 10, color: C.muted, textTransform: "uppercase", letterSpacing: 1.5, margin: 0, fontFamily: "Inter, sans-serif" }}>{label}</p>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <span style={{ fontSize: 16, fontWeight: 900, color, fontFamily: "Inter, sans-serif" }}>{last}{unit}</span>
          <span style={{ fontSize: 11, color: trend >= 0 ? C.green : C.red, background: trend >= 0 ? "rgba(22,163,74,0.1)" : "rgba(204,41,54,0.1)", borderRadius: 6, padding: "2px 6px", fontFamily: "Inter, sans-serif" }}>
      ...
      <ResponsiveContainer width="100%" height={height}>
        <AreaChart data={data}>
          ...
          <XAxis dataKey="date" tick={{ fill: C.muted, fontSize: 8, fontFamily: "Inter" }} tickFormatter={d => d.slice(5)} axisLine={false} tickLine={false} />
          <YAxis tick={{ fill: C.muted, fontSize: 9, fontFamily: "Inter" }} width={30} domain={["auto", "auto"]} axisLine={false} tickLine={false} />
          <Tooltip contentStyle={{ background: C.surface, border: `1px solid ${color}`, borderRadius: 10, fontSize: 11, fontFamily: "Inter" }} formatter={v => [`${v}${unit}`, label]} />
...
const Toggle = ({ value, onChange, label }) => (
  <div onClick={() => onChange(!value)} style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", background: value ? C.redLight : C.surfaceAlt, border: `1px solid ${value ? C.red : C.border}`, borderRadius: 12, padding: "10px 14px", transition: "all 0.2s", userSelect: "none" }}>
    ...
    <span style={{ fontSize: 13, color: value ? C.red : C.muted, fontWeight: value ? 600 : 400, fontFamily: "Inter, sans-serif" }}>{label}</span>
  </div>
);

const Field = ({ label, children }) => (
  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
    <label style={{ fontSize: 10, color: C.muted, textTransform: "uppercase", letterSpacing: 1.5, fontFamily: "Inter, sans-serif" }}>{label}</label>
    {children}
  </div>
);
...
const ST = ({ children }) => (
  <p style={{ fontSize: 10, color: C.red, textTransform: "uppercase", letterSpacing: 2, marginBottom: 12, marginTop: 0, fontWeight: 700, fontFamily: "Inter, sans-serif" }}>{children}</p>
);

const AlertBox = ({ type, msg }) => (
  <div style={{ background: type === "danger" ? "rgba(204,41,54,0.08)" : "rgba(234,88,12,0.08)", border: `1px solid ${type === "danger" ? C.red : C.orange}`, borderRadius: 12, padding: "10px 14px", marginBottom: 10, fontSize: 12, color: type === "danger" ? C.red : C.orange, lineHeight: 1.5, fontFamily: "Inter, sans-serif" }}>{msg}</div>
);

const AdviceBox = ({ msg }) => (
  <div style={{ background: "rgba(22,163,74,0.06)", border: "1px solid rgba(22,163,74,0.2)", borderRadius: 12, padding: "10px 14px", marginBottom: 10, fontSize: 12, color: C.green, lineHeight: 1.5, fontFamily: "Inter, sans-serif" }}>{msg}</div>
);
...
const DraggableList = ({ items, onReorder, renderItem }) => {
  ...
  return (
    <div>
      {items.map((item, idx) => (
        <div key={item.id} draggable onDragStart={e => handleDragStart(e, idx)} onDragOver={e => handleDragOver(e, idx)} onDrop={e => handleDrop(e, idx)} onDragEnd={() => { setDragging(null); setDragOver(null); }}
          style={{ opacity: dragging === idx ? 0.4 : 1, borderTop: dragOver === idx && dragging !== idx ? `2px solid ${C.red}` : "2px solid transparent", transition: "border 0.15s, opacity 0.15s", cursor: "grab" }}>
          {renderItem(item, idx)}
        </div>
      ))}
    </div>
...
const DotsMenu = ({ onRename, onColor, onDelete, color }) => {
  ...
  useEffect(() => {
    const handler = (e) => { if (ref.current && !ref.current.contains(e.target)) { setOpen(false); setShowColors(false); } };
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <button onClick={() => setOpen(o => !o)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 18, color: C.muted, padding: "4px 8px", borderRadius: 8 }}>···</button>
      {open && (
        <div style={{ position: "absolute", right: 0, top: 32, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, boxShadow: "0 8px 24px rgba(0,0,0,0.12)", zIndex: 100, minWidth: 170, overflow: "hidden" }}>
          {showColors ? (
            <div style={{ padding: 12 }}>
              ...
              <input type="color" value={color} onChange={e => onColor(e.target.value)} style={{ width: "100%", height: 32, marginTop: 8, border: "none", borderRadius: 8, cursor: "pointer" }} />
            </div>
          ) : (
            <>
              <button onClick={() => setShowColors(true)} style={{ display: "block", width: "100%", padding: "12px 16px", background: "none", border: "none", cursor: "pointer", fontSize: 13, textAlign: "left", fontFamily: "Inter, sans-serif", color: C.text }}>🎨 Changer couleur</button>
              <button onClick={() => { onRename(); setOpen(false); }} style={{ display: "block", width: "100%", padding: "12px 16px", background: "none", border: "none", cursor: "pointer", fontSize: 13, textAlign: "left", fontFamily: "Inter, sans-serif", color: C.text, borderTop: `1px solid ${C.border}` }}>✏️ Renommer</button>
              <button onClick={() => { onDelete(); setOpen(false); }} style={{ display: "block", width: "100%", padding: "12px 16px", background: "none", border: "none", cursor: "pointer", fontSize: 13, textAlign: "left", fontFamily: "Inter, sans-serif", color: C.red, borderTop: `1px solid ${C.border}` }}>🗑️ Supprimer</button>
            </>
          )}
        </div>
      )}
...
const PageTransition = ({ children, pageKey }) => {
  ...
  useEffect(() => { setVisible(false); const t = setTimeout(() => setVisible(true), 60); return () => clearTimeout(t); }, [pageKey]);
  return (
    <div style={{ opacity: visible ? 1 : 0, transition: "opacity 0.25s ease", transform: visible ? "translateY(0)" : "translateY(6px)" }}>
      {children}
    </div>
  );
};

// ── TAB BUTTON WITH SCALE ANIMATION ───────────────────────────────────────
const TrackTabBtn = ({ active, onClick, children }) => {
  const [pressed, setPressed] = useState(false);
  return (
    <button
      onClick={() => { setPressed(true); setTimeout(() => setPressed(false), 200); onClick(); }}
      style={{ flexShrink: 0, padding: "7px 14px", borderRadius: 20, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600, background: active ? C.red : C.surfaceAlt, color: active ? "#fff" : C.muted, fontFamily: "Inter, sans-serif", transform: pressed ? "scale(1.08)" : "scale(1)", transition: "transform 0.15s ease, background 0.2s" }}
    >{children}</button>
  );
};
...
export default function App() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("kojihlife_v5");
      if (raw) {
  ...
  const saveAll = useCallback((h, t, g, p, pr) => {
    localStorage.setItem("kojihlife_v5", JSON.stringify({ history: h, todos: t, goals: g, patrimoine: p, profile: pr }));
  }, []);
  ...
  const globalStyle = { fontFamily: "Inter, sans-serif" };

  return (
    <div style={{ ...globalStyle, minHeight: "100vh", background: C.bg, color: C.text, maxWidth: 480, margin: "0 auto", paddingBottom: 80 }}>
      <style>{`
      ...
      <div style={{ padding: "20px 20px 14px", borderBottom: `1px solid ${C.border}`, background: C.surface, boxShadow: "0 2px 8px rgba(0,0,0,0.06)", position: "sticky", top: 0, zIndex: 10 }}>
        ...
        {intel.alerts.length > 0 && <AlertBox type={intel.alerts[0].type} msg={intel.alerts[0].msg} />}
        {intel.alerts.length === 0 && intel.advice.length > 0 && <AdviceBox msg={intel.advice[0]} />}
      </div>

      <div style={{ padding: 16 }}>
        <PageTransition pageKey={nav}>
          ...
          {nav === "today" && (
            <div>
              <Card>
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <RadarChart data={radar}>
                    ...
                    <PolarAngleAxis dataKey="s" tick={{ fill: C.muted, fontSize: 11, fontFamily: "Inter" }} />
              ...
              <Card>
                ...
                {todos.filter(t => t.date === today.date).length === 0 && <p style={{ fontSize: 12, color: C.muted, margin: 0 }}>Aucune tâche → onglet Tracker</p>}
              </Card>
            </div>
          )}
          ...
          {nav === "track" && (
            <div>
              <div style={{ display: "flex", overflowX: "auto", gap: 6, marginBottom: 16, scrollbarWidth: "none" }}>
                {TRACK_TABS.map(t => (
                  <TrackTabBtn key={t.id} active={trackTab === t.id} onClick={() => setTrackTab(t.id)}>
                    {t.icon} {t.label}
                  </TrackTabBtn>
                ))}
              </div>

              <PageTransition pageKey={trackTab}>
                ...
                {trackTab === "sport" && (
                  <div>
                    {intel.todayRec === "rest" ? <AlertBox type="warning" msg={`💤 ${intel.consecutiveSport} jours consécutifs — repos actif recommandé.`} /> : intel.todayRec ? <AdviceBox msg={`🎯 Recommandation : ${intel.todayRec}`} /> : null}
              ...
              <button onClick={saveDay} style={{ width: "100%", padding: "14px", borderRadius: 14, border: "none", cursor: "pointer", background: saved ? C.green : C.red, color: "#fff", fontSize: 15, fontWeight: 800, transition: "all 0.3s", marginTop: 4, fontFamily: "Inter, sans-serif" }}>
          ...
          {nav === "money" && (
            <div>
              ...
              {patrimoine.some(p => p.amount > 0) && (
                <Card>
                  ...
                  <ResponsiveContainer width="100%" height={180}>
                    <PieChart>
                      <Pie data={patrimoine.filter(p => p.amount > 0)} dataKey="amount" nameKey="name" cx="50%" cy="50%" outerRadius={70} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} fontSize={10}>
              ...
              <Card>
                ...
                <DraggableList
                  items={patrimoine}
                  onReorder={p => { setPatrimoine(p); saveAll(history, todos, goals, p, profile); }}
                  renderItem={(p) => (
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10, padding: "10px 12px", background: C.surfaceAlt, borderRadius: 12, borderLeft: `4px solid ${p.color}` }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 4 }}>
                          <input type="number" value={p.amount} onChange={e => updatePoche(p.id, "amount", +e.target.value)} style={{ background: "transparent", border: "none", outline: "none", fontSize: 16, fontWeight: 800, color: p.color, width: 130, fontFamily: "Inter, sans-serif" }} />
                          <span style={{ fontSize: 12, color: C.muted }}>€</span>
                        </div>
                      </div>
                      <DotsMenu
                        color={p.color}
                        onRename={() => setRenamingPoche(p.id)}
                        onColor={col => updatePoche(p.id, "color", col)}
                        onDelete={() => deletePoche(p.id)}
                      />
                    </div>
                  )}
                />
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 8 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    ...
                    <button onClick={addPoche} style={{ flex: 1, padding: "11px", background: C.red, color: "#fff", border: "none", borderRadius: 10, fontWeight: 700, cursor: "pointer", fontSize: 13, fontFamily: "Inter, sans-serif" }}>+ Ajouter une poche</button>
                  </div>
                </div>
              </Card>
            ...
            <div>
              <Card>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  <input value={newGoal.category} onChange={e => setNewGoal(p => ({ ...p, category: e.target.value }))} placeholder="Catégorie (Nutrition, Sport...)" style={inp} />
                  ...
                  {newGoal.sourceId !== "manual" && (
                    <>
                      ...
                      <Toggle value={newGoal.reverse} onChange={v => setNewGoal(p => ({ ...p, reverse: v }))} label="Objectif : descendre sous la cible (ex: perdre du poids)" />
                    </>
                  )}
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    <Field label="Date de début"><input type="date" value={newGoal.startDate} onChange={e => setNewGoal(p => ({ ...p, startDate: e.target.value }))} style={inp} /></Field>
                    <Field label="Date de fin"><input type="date" value={newGoal.endDate} onChange={e => setNewGoal(p => ({ ...p, endDate: e.target.value }))} style={inp} /></Field>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                    <p style={{ fontSize: 11, color: C.muted, margin: 0 }}>Couleur :</p>
                    {GOAL_COLORS.map(col => (
                      <div key={col} onClick={() => setNewGoal(p => ({ ...p, color: col }))} style={{ width: 26, height: 26, borderRadius: "50%", background: col, cursor: "pointer", border: newGoal.color === col ? `3px solid ${C.text}` : "3px solid transparent" }} />
                    ))}
                  </div>
                  <button onClick={addGoal} style={{ background: newGoal.color, color: "#fff", border: "none", borderRadius: 10, padding: "12px", fontWeight: 700, cursor: "pointer", fontSize: 14, fontFamily: "Inter, sans-serif" }}>+ Ajouter l'objectif</button>
                </div>
              </Card>

              <DraggableList
                items={computedGoals}
                onReorder={g => { setGoals(g); saveAll(history, todos, g, patrimoine, profile); }}
                renderItem={(g) => {
                  const src = DATA_SOURCES.find(s => s.id === g.sourceId);
                  const daysLeft = g.endDate ? Math.max(0, Math.round((new Date(g.endDate) - new Date()) / (1000 * 60 * 60 * 24))) : null;
                  return (
                    <Card key={g.id}>
                      ...
                      <div style={{ height: 10, background: C.surfaceAlt, borderRadius: 5, marginBottom: g.sourceId === "manual" ? 10 : 0 }}>
                      ...
                      {g.sourceId === "manual" && (
                        <input type="range" min={0} max={100} value={g.computedProgress} onChange={e => updateGoalField(g.id, "manualProgress", +e.target.value)} style={{ width: "100%", accentColor: g.color, marginTop: 8 }} />
                      )}
            ...
            <div>
              <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
                {[["7","7j"],["30","30j"],["90","3 mois"],["365","1 an"]].map(([v, l]) => (
                  <button key={v} onClick={() => setStatRange(v)} style={{ flex: 1, padding: "8px 4px", borderRadius: 10, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 600, background: statRange === v ? C.red : C.surfaceAlt, color: statRange === v ? "#fff" : C.muted, fontFamily: "Inter, sans-serif" }}>{l}</button>
                ))}
              </div>
              ...
              <Card>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  ...
                  <Field label="Date de naissance">
                    <input type="date" value={profile.dob || ""} onChange={e => updateProfile("dob", e.target.value)} style={inp} />
                  </Field>
                  {age !== null && (
        ...
        {NAV.map(n => {
          ...
          return (
            <button key={n.id} onClick={() => setNav(n.id)} style={{ flex: 1, padding: "10px 4px 8px", border: "none", background: "transparent", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, transition: "all 0.2s" }}>
              {Icons[n.icon](active ? C.red : C.black)}
              <span style={{ fontSize: 9, fontWeight: 600, color: active ? C.red : C.black, textTransform: "uppercase", letterSpacing: 0.5, fontFamily: "Inter, sans-serif" }}>{n.label}</span>
              {active && <div style={{ width: 4, height: 4, borderRadius: "50%", background: C.red }} />}
            </button>
          );
        })}
      </div>
